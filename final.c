#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <time.h>

/*
    1= barrage//2= house//3=off lamp//4=on lamp//5=open hole//6=escape//7=jw//8=wg//9=il//10=js//11=sh//12=jb//13=ms//14=sg
    //15=empty//16= close hole*/
typedef struct {
    int block_number;
    int number;
    int on_off;
} block;

struct node {
    int info;
    struct node *next;
};

struct node *create_node(int i) {
    struct node *nn;
    nn = (struct node *) malloc(sizeof(struct node));
    if (nn == NULL)
        return NULL;
    nn->info = i;
    nn->next = NULL;
    return nn;
}

void add_end(struct node *list, struct node *new_node) {
    struct node *current;
    for (current = list; current->next != NULL; current = current->next);
    current->next = new_node;
    new_node->next = NULL;
}

/*
{1,"barrage",'B'}
{2,"house",'H'}
{3,"off lamp",'offL'}
{4,"on lamp",'on_L'}
{5,"open hole",'O'}
{6,"escape",'E'}
{7,"JW",'JW'},
{8,"WG",'WG'}
{9, "IL",'IL'}
{10,"JS",'JS'}
{11,"SH",'SH'}
{12,"JB",'JB'}
{13,"MS",'MS'}
{14,"Sg",'SG'}
{15,"empty",' '}
{16,"close hole",'x'}

*/
char *convert(int n) {
    if (n == 1)
        return "B";
    else if (n == 2)
        return "H";
    else if (n == 3)
        return "offL";
    else if (n == 4)
        return "on_L";
    else if (n == 5)
        return "O";
    else if (n == 6)
        return "E";
    else if (n == 7)
        return "JW";
    else if (n == 8)
        return "WG";
    else if (n == 9)
        return "IL";
    else if (n == 10)
        return "JS";
    else if (n == 11)
        return "SH";
    else if (n == 12)
        return "JB";
    else if (n == 13)
        return "MS";
    else if (n == 14)
        return "SG";
    else if (n == 15)
        return " ";
    else if (n == 16)
        return "X";
}

block map[11][15] = {{{2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2}},

                     {{2},
                             {2},
                             {1},
                             {2},
                             {2},
                             {2},
                             {2},
                             {15},
                             {5},
                             {13},
                             {2},
                             {2},
                             {6},
                             {2},
                             {2}},

                     {{2},
                             {15},
                             {15},
                             {16},
                             {2},
                             {15},
                             {15},
                             {15},
                             {3},
                             {15},
                             {2},
                             {4},
                             {15},
                             {2},
                             {2}},

                     {{2},
                             {15},
                             {4},
                             {15},
                             {15},
                             {15},
                             {2},
                             {11},
                             {2},
                             {15},
                             {15},
                             {15},
                             {15},
                             {15},
                             {2}},

                     {{2},
                             {7},
                             {15},
                             {15},
                             {2},
                             {9},
                             {4},
                             {15},
                             {5},
                             {15},
                             {2},
                             {2},
                             {2},
                             {5},
                             {2}},

                     {{2},
                             {15},
                             {2},
                             {2},
                             {15},
                             {15},
                             {2},
                             {15},
                             {2},
                             {12},
                             {15},
                             {15},
                             {2},
                             {14},
                             {2}},

                     {{2},
                             {5},
                             {2},
                             {15},
                             {2},
                             {15},
                             {5},
                             {10},
                             {4},
                             {15},
                             {2},
                             {15},
                             {15},
                             {15},
                             {2}},

                     {{2},
                             {2},
                             {15},
                             {4},
                             {15},
                             {15},
                             {2},
                             {15},
                             {2},
                             {15},
                             {15},
                             {15},
                             {4},
                             {15},
                             {2}},

                     {{2},
                             {2},
                             {15},
                             {2},
                             {2},
                             {8},
                             {3},
                             {15},
                             {15},
                             {2},
                             {2},
                             {2},
                             {16},
                             {2},
                             {2}},

                     {{2},
                             {2},
                             {6},
                             {2},
                             {2},
                             {2},
                             {5},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {1},
                             {2},
                             {2}},

                     {{2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2},
                             {2}}};

void print_list(struct node *list) {
    struct node *current = list->next;
    while (current != NULL) {
        printf("%d ", current->info);
        current = current->next;
    }
    printf("\n");
}

void print_in_mid(char a[5], int x) {
    int leni = 0;
    int temp = x;
    while (temp > 0) {
        temp /= 10;
        leni++;
    }
    if (leni == 3) {
        int len = strlen(a);
        int space1 = (4 - len) / 2;
        printf("%d", x);
        for (int i = 0; i < space1; i++) {
            printf(" ");
        }
        printf("%s", a);
        for (int i = 0; i < (4 - space1 - len); ++i) {
            printf(" ");
        }
    }
    if (leni == 2) {
        int len = strlen(a);
        int space1 = (5 - len) / 2;
        printf("%d", x);
        for (int i = 0; i < space1; i++) {
            printf(" ");
        }
        printf("%s", a);
        for (int i = 0; i < (5 - space1 - len); ++i) {
            printf(" ");
        }
    }
    if (leni == 1) {
        int len = strlen(a);
        int space1 = (6 - len) / 2;
        printf("%d", x);
        for (int i = 0; i < space1; i++) {
            printf(" ");
        }
        printf("%s", a);
        for (int i = 0; i < (6 - space1 - len); ++i) {
            printf(" ");
        }
    }
}


void print_map() {
    for (int j = 0; j < 7; j++) {
        printf("         _____");
    }
    int flag1 = 10, flag2 = 0, flag3 = 11, flag4 = 1;
    for (int i = 0; i < 23; i++) {
        if (i % 2 == 0) {
            if (i == 0) {
                printf("\n ");
                for (int j = 0; j < 7; j++) {
                    printf("       /     \\");
                }
                printf("\n  ");
                for (int j = 0; j < 7; j++) {
                    printf("_____/       \\");

                }
                printf("_____");
            } else {
                printf("\n");
                for (int j = 0; j < 8; j++) {
                    printf("\\");
                    print_in_mid(convert(map[flag1 - flag2][(2 * j)].block_number), map[flag1 - flag2][(2 * j)].number);
                    printf("/     ");
                }
                flag1++;
                flag2 += 2;
                printf("\n ");
                for (int j = 0; j < 8; j++) {
                    if (j < 7) {
                        printf("\\_____/    ");
                        printf("   ");
                    } else
                        printf("\\_____/       ");
                }
            }
        } else {
            printf("\n ");
            for (int j = 0; j < 8; j++) {
                if (j < 7) {
                    printf("/     \\");
                    print_in_mid(convert(map[flag3 - flag4][(2 * j) + 1].block_number),
                                 map[flag3 - flag4][(2 * j) + 1].number);
                    printf("");
                } else
                    printf("/     \\       ");
            }
            flag3++;
            flag4 += 2;
            printf("\n");
            for (int j = 0; j < 7; j++) {
                printf("/       \\_____");
            }
            printf("/       \\");
        }
    }
}


void print_list_with_char(struct node *list) {
    struct node *current = list->next;
    while (current != NULL) {
        printf("%d)", current->info);
        if (current->info == 1)
            printf("SH: move 1 to 3 houses then show suspect card\n");
        else if (current->info == 2)
            printf("JB: move 1 to 3 houses before or after change location cap of hole\n");
        else if (current->info == 3)
            printf("WG: move 1 to 3 houses then or change location with other character\n");
        else if (current->info == 4)
            printf("IL: move 1 to 3 houses then change position of barrage \n");
        else if (current->info == 5)
            printf("JW: move 1 to 3 houses then chose position for clarify\n");
        else if (current->info == 6)
            printf("MS: move 1 to 4 houses but can transit engaged location\n");
        else if (current->info == 7)
            printf("SG: move 1 to 3 houses before or after altogether change 3 location of other character \n");
        else if (current->info == 8)
            printf("JS: move 1 to 3 houses after or before turn off one lamp and turn on other lamp\n");
        current = current->next;
    }
    printf("\n");
}

struct node *print_first_half_list(struct node *list) {
    int flag = 1;
    struct node *list2 = NULL;
    list2 = create_node(0);
    struct node *current = list->next;
    while (current != NULL && flag <= 4) {
        add_end(list2, create_node(current->info));
        current = current->next;
        flag++;
    }
    return list2;
}

struct node *print_second_half_list(struct node *list) {
    int flag = 1;
    struct node *list2 = NULL;
    list2 = create_node(0);
    struct node *current = list->next;
    while (current != NULL) {
        if (flag > 4) {
            add_end(list2, create_node(current->info));
            current = current->next;
            flag++;
        } else {
            current = current->next;
            flag++;
        }
    }
    return list2;
}


struct node *random1() {
    struct node *list = NULL;
    srand(time(NULL));
    int rand_num = 0;
    list = create_node(rand_num);
    int cnt = 1;
    struct node *current;
    while (cnt <= 8) {
        int rand_number = rand() % 8 + 1;
        int flag = 0;
        for (current = list; current != NULL; current = current->next) {
            if (rand_number == current->info) {
                flag = 1;
            }
        }
        if (flag == 0) {
            add_end(list, create_node(rand_number));
            cnt++;
        }
    }
    return list;
}

int mr_jack;

void who_is_jack() {
    srand(time(NULL));
    int rand_number = rand() % 8 + 1;
    if (rand_number == 1) {
        printf("mr_jack is sh\n");
        mr_jack = 1;
    } else if (rand_number == 2) {
        printf("mr_jack is jb\n");
        mr_jack = 2;
    } else if (rand_number == 3) {
        printf("mr_jack is wg\n");
        mr_jack = 3;
    } else if (rand_number == 4) {
        printf("mr_jack is il\n");
        mr_jack = 4;
    } else if (rand_number == 5) {
        printf("mr_jack is jw\n");
        mr_jack = 5;
    } else if (rand_number == 6) {
        printf("mr_jack is ms\n");
        mr_jack = 6;
    } else if (rand_number == 7) {
        printf("mr_jack is sg\n");
        mr_jack = 7;
    } else if (rand_number == 8) {
        printf("mr_jack is js\n");
        mr_jack = 8;
    }
}

struct node *suspect_list = NULL;

void suspect() {
    suspect_list = create_node(0);
    for (int i = 1; i <= 8; i++) {
        if (i != mr_jack) {
            add_end(suspect_list, create_node(i));
        }
    }
}

void delete(struct node **list, int num) {
    struct node *temp;
    if ((*list)->info == num) {
        temp = *list;
        *list = (*list)->next;
        free(temp);
    } else {
        struct node *current = *list;
        while (current->next != NULL) {
            if (current->next->info == num) {
                temp = current->next;
                current->next = current->next->next;
                free(temp);
                break;
            } else
                current = current->next;
        }
    }
}

void show_suspect() {
    struct node *current = suspect_list->next;
    if (current->info != mr_jack) {
        if (current->info == 1) {
            printf("mr_jack is not sh\n");
        }
        if (current->info == 2) {
            printf("mr_jack is not jb\n");
        }
        if (current->info == 3) {
            printf("mr_jack is not wg\n");
        }
        if (current->info == 4) {
            printf("mr_jack is not il\n");
        }
        if (current->info == 5) {
            printf("mr_jack is not jw\n");
        }
        if (current->info == 6) {
            printf("mr_jack is not ms\n");
        }
        if (current->info == 7) {
            printf("mr_jack is not sg\n");
        }
        if (current->info == 8) {
            printf("mr_jack is not js\n");
        }
    }
    delete(&suspect_list, current->info);
}

int rouund = 1;

void start_game() {
    printf("welcome to mr_jack game\n");
    printf("if you want load game enter 1 and new game enter other number\n");
    int load;
    scanf("%d", &load);
    if (load == 1) {
        FILE *ptr;
        ptr = fopen("test.bin", "rb");
        fread(&rouund, sizeof(rouund), 1, ptr);
        int map_char;
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 15; j++) {
                fread(&map_char, sizeof(map_char), 1, ptr);
                map[i][j].number = map_char;
            }
        }
    }
    printf("enter your selection:\n1) player1:mr_jack  player2:sleuth\n2) player1:sleuth  player2:mr_jack\n");
    int select;
    scanf("%d", &select);
    if (select != 1 && select != 2) {
        printf("selection is invalid");
        start_game();
    }
    who_is_jack();
    suspect();
}


void move(int location) {
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == location) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    printf("if you want to move, enter the appropriate number:\n"
           "1)up\n"
           "2)down\n"
           "3)up_right\n"
           "4)up_left\n"
           "5)down_right\n"
           "6)down_left\n");
    int select;
    scanf("%d", &select);
    if (select == 1) {
        if (map[j][i - 1].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j][i - 1].block_number;
            map[j][i - 1].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j][i - 1].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;

        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }
    } else if (select == 2) {
        if (map[j - 2][i - 1].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j - 2][i - 1].block_number;
            map[j - 2][i - 1].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j - 2][i - 1].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;
        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }

    } else if (select == 3) {
        if (map[j][i].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j][i].block_number;
            map[j][i].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j][i - 1].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;
        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }


    } else if (select == 4) {
        if (map[j][i - 2].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j][i - 2].block_number;
            map[j][i - 2].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j][i - 2].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;

        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }


    } else if (select == 5) {
        if (map[j - 1][i].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j - 1][i].block_number;
            map[j - 1][i].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j - 1][i].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;
        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }
    } else if (select == 6) {
        if (map[j - 1][i - 2].block_number == 15) {
            int block_number_temp;
            block_number_temp = map[j - 1][i - 2].block_number;
            map[j - 1][i - 2].block_number = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = block_number_temp;
        } else if (map[j - 1][i - 2].block_number == 5) {
            printf("chose hole that you want to out\n");
            int hole;
            scanf("%d", &hole);
            int i2, j2;
            for (int h = 1; h <= 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == hole) {
                        j2 = h + 1;
                        i2 = k + 1;
                    }
                }
            }
            int block_number_temp;
            block_number_temp = map[j - 1][i - 1].block_number;
            map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
            map[j2 - 1][i2 - 1].block_number = block_number_temp;
        } else {
            printf("invalid move\n");
            move(map[j - 1][i - 1].number);
        }

    } else {
        printf("incorrect input");
        move(location);
    }
    print_map();
    printf("\n");
}

void turn_on_way() {
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].block_number == 7) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    printf("if you want to move, enter the appropriate number:\n"
           "1)up\n"
           "2)down\n"
           "3)up_right\n"
           "4)up_left\n"
           "5)down_right\n"
           "6)down_left\n");
    int select;
    scanf("%d", &select);
    if (select == 1) {
        while (j < 12) {
            map[j][i - 1].on_off = 1;
            j++;
        }

    } else if (select == 2) {
        while (j > 0) {
            map[j - 2][i - 1].on_off = 1;
            j--;
        }
    } else if (select == 3) {
        while (j < 12 && i < 16) {
            map[j][i].on_off = 1;
            j++;
            i++;
        }
    } else if (select == 4) {
        while (j < 12 && i > 0) {
            map[j][i - 2].on_off = 1;
            j++;
            i--;
        }
    } else if (select == 5) {
        while (i < 16) {
            map[j - 1][i].on_off = 1;
            i++;
        }
    } else if (select == 6) {
        while (i > 0) {
            map[j - 1][i - 2].on_off = 1;
            i--;
        }
    }
}

void turn_on_lamp() {
    printf("enter the number of lamp that you want to change to off\n");
    int offlamp;
    scanf("%d\n", &offlamp);
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == offlamp) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 3;
    printf("enter the number of lamp that you want to change to on\n");
    int onlamp;
    scanf("%d\n", &onlamp);
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == onlamp) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 4;
}


void change_token() {
    printf("enter the number of barrage that you want to change to escape\n");
    int barrage;
    scanf("%d\n", &barrage);
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == barrage) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 6;
    printf("enter the number of escape that you want to change to barrage\n");
    int escape;
    scanf("%d\n", &escape);
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == escape) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 1;
}

void action_sh() {
    printf("enter number of move (1 to 3):\n");
    int move_num;
    scanf("%d", &move_num);
    if (move_num > 3) {
        printf("invalid number of move\n");
        while (move_num > 3) {
            printf("invalid number of move\n");
            scanf("%d", &move_num);
        }
    }
    int i = 0, j = 0;
    for (int x = 0; x < move_num; x++) {
        for (int h = 0; h < 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 11) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        move(map[j - 1][i - 1].number);
    }
    show_suspect();
}

void change_hole() {
    printf("enter the number of open hole that you want to change to close\n");
    int open;
    scanf("%d\n", &open);
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == open) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 16;
    printf("enter the number of close hole that you want to change to open\n");
    int close;
    scanf("%d\n", &close);
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == close) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    map[j - 1][i - 1].block_number = 5;
}

void action_jb() {
    printf("if you want to move after soot:enter 1\nbackwards:enter 2\n");
    int chose;
    scanf("%d", &chose);
    if (chose != 1 && chose != 2) {
        printf("invalid chose\n");
        action_jb();
    }
    if (chose == 1) {
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 14) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
        change_hole();
    }
    if (chose == 2) {
        change_hole();
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 14) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
    }
}

void change_location() {
    printf("enter the number of house of char that you want to change\n");
    int house_of_change;
    scanf("%d\n", &house_of_change);
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].block_number == 8) {
                j = h + 1;
                i = k + 1;
            }
        }
    }
    int i2 = 0, j2 = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].number == house_of_change) {
                j2 = h + 1;
                i2 = k + 1;
            }
        }
    }
    int temp;
    temp = map[j - 1][i - 1].block_number;
    map[j - 1][i - 1].block_number = map[j2 - 1][i2 - 1].block_number;
    map[j2 - 1][i2 - 1].block_number = temp;
}

void action_wg() {
    printf("if you want to move:enter 1\nif you want change location with other character:enter 2\n");
    int chose;
    scanf("%d", &chose);
    if (chose != 1 && chose != 2) {
        printf("invalid chose\n");
        action_wg();
    }
    if (chose == 1) {
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 8) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
    }
    if (chose == 2) {
        printf("enter location that you want to go:\n");
        int select;
        scanf("%d", &select);
        int i = 0, j = 0;
        for (int h = 0; h < 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].number == select) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].block_number == 7 || map[j - 1][i - 1].block_number == 8 ||
            map[j - 1][i - 1].block_number == 9 || map[j - 1][i - 1].block_number == 10 ||
            map[j - 1][i - 1].block_number == 11 || map[j - 1][i - 1].block_number == 12 ||
            map[j - 1][i - 1].block_number == 13 || map[j - 1][i - 1].block_number == 14) {
            change_location();

        } else {
            printf("invalid change\n");
            printf("enter location that you want to go:\n");
            scanf("%d", &select);
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].number == select) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            if (map[j - 1][i - 1].block_number == 7 || map[j - 1][i - 1].block_number == 8 ||
                map[j - 1][i - 1].block_number == 9 || map[j - 1][i - 1].block_number == 10 ||
                map[j - 1][i - 1].block_number == 11 || map[j - 1][i - 1].block_number == 12 ||
                map[j - 1][i - 1].block_number == 13 || map[j - 1][i - 1].block_number == 14) {
                change_location();

            }
        }
    }
}

void action_il() {
    printf("enter number of move (1 to 3):\n");
    int move_num;
    scanf("%d", &move_num);
    if (move_num > 3) {
        printf("invalid number of move\n");
        while (move_num > 3) {
            printf("invalid number of move\n");
            scanf("%d", &move_num);
        }
    }
    int i = 0, j = 0;
    for (int x = 0; x < move_num; x++) {
        for (int h = 0; h < 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 9) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        move(map[j - 1][i - 1].number);
    }
    change_token();
}


void action_jw() {
    printf("enter number of move (1 to 3):\n");
    int move_num;
    scanf("%d", &move_num);
    if (move_num > 3) {
        printf("invalid number of move\n");
        action_jw();
    }
    int i = 0, j = 0;
    for (int x = 0; x < move_num; x++) {
        for (int h = 0; h < 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 7) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        move(map[j - 1][i - 1].number);
    }
    turn_on_way();
}


void action_ms() {
    printf("enter number of move (1 to 4):\n");
    int move_num;
    scanf("%d", &move_num);
    if (move_num > 4) {
        printf("invalid number of move\n");
        action_ms();
    }
    int i = 0, j = 0;
    for (int x = 0; x < move_num; x++) {
        for (int h = 0; h < 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 13) {
                    j = h + 1;
                    i = k + 1;
                }
            }
            move(map[j - 1][i - 1].number);
        }
    }
}


void action_sg() {
    printf("if you want to move after soot:enter 1\nbackwards:enter 2\n");
    int chose;
    scanf("%d", &chose);
    if (chose != 1 && chose != 2) {
        printf("invalid chose\n");
        action_sg();
    }
    if (chose == 1) {
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 14) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
        //  soot();
    }
    if (chose == 2) {
        //   soot();
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 14) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
    }
}

void action_js() {
    printf("if you want to move after turn on lamp:enter 1\nbackwards:enter 2\n");
    int chose;
    scanf("%d", &chose);
    if (chose != 1 && chose != 2) {
        printf("invalid chose\n");
        action_js();
    }
    if (chose == 1) {
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 10) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
        turn_on_lamp();
    }
    if (chose == 2) {
        turn_on_lamp();
        printf("enter number of move (1 to 3):\n");
        int move_num;
        scanf("%d", &move_num);
        if (move_num > 3) {
            printf("invalid number of move\n");
            while (move_num > 3) {
                printf("invalid number of move\n");
                scanf("%d", &move_num);
            }
        }
        int i = 0, j = 0;
        for (int x = 0; x < move_num; x++) {
            for (int h = 0; h < 11; h++) {
                for (int k = 0; k < 15; k++) {
                    if (map[h][k].block_number == 10) {
                        j = h + 1;
                        i = k + 1;
                    }
                }
            }
            move(map[j - 1][i - 1].number);
        }
    }
}


void action(int n) {
    if (n == 1)
        action_sh();
    if (n == 2)
        action_jb();
    if (n == 3)
        action_wg();
    if (n == 4)
        action_il();
    if (n == 5)
        action_jw();
    if (n == 6)
        action_ms();
    if (n == 7)
        action_sg();
    if (n == 8)
        action_js();
}


void handle_odd_round(struct node *list) {
    print_list_with_char(list);
    for (int i = 0; i < 4; i++) {
        if (i == 0 || i == 3) {
            int flag = 0;
            while (flag == 0) {
                printf("sleuth enter the number of character\n");
                int num;
                scanf("%d", &num);
                struct node *curr;
                for (curr = list->next; curr != NULL;) {
                    if (num == curr->info)
                        flag = 1;
                    curr = curr->next;
                }
                if (flag == 0) {
                    printf("invalid input\n");
                } else {
                    action(num);
                    delete(&list, num);

                }
            }
        } else {
            int flag = 0;
            while (flag == 0) {
                printf("mr_jack enter the number of character\n");
                int num;
                scanf("%d", &num);
                struct node *curr;
                for (curr = list->next; curr != NULL;) {
                    if (num == curr->info)
                        flag = 1;
                    curr = curr->next;
                }
                if (flag == 0) {
                    printf("invalid input\n");
                } else {
                    action(num);
                    delete(&list, num);

                }
            }
        }
    }
}

void handle_even_round(struct node *list) {
    print_list_with_char(list);
    for (int i = 0; i < 4; i++) {
        if (i == 0 || i == 3) {
            int flag = 0;
            while (flag == 0) {
                printf("mr_jack enter the number of character\n");
                int num;
                scanf("%d", &num);
                struct node *curr;
                for (curr = list->next; curr != NULL;) {
                    if (num == curr->info)
                        flag = 1;
                    curr = curr->next;
                }
                if (flag == 0) {
                    printf("invalid input\n");
                } else {
                    action(num);
                    delete(&list, num);

                }
            }
        } else {
            int flag = 0;
            while (flag == 0) {
                printf("sleuth enter the number of character\n");
                int num;
                scanf("%d", &num);
                struct node *curr;
                for (curr = list->next; curr != NULL;) {
                    if (num == curr->info)
                        flag = 1;
                    curr = curr->next;
                }
                if (flag == 0) {
                    printf("invalid input\n");
                } else {
                    action(num);
                    delete(&list, num);

                }
            }
        }
    }

}

void state_mr_jack() {
    if (mr_jack == 1) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 11) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 2) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 12) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 3) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 8) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 4) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 9) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }

    if (mr_jack == 5) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 4) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 6) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 13) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 7) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 14) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
    if (mr_jack == 8) {
        int i = 0, j = 0;
        for (int h = 1; h <= 11; h++) {
            for (int k = 0; k < 15; k++) {
                if (map[h][k].block_number == 10) {
                    j = h + 1;
                    i = k + 1;
                }
            }
        }
        if (map[j - 1][i - 1].on_off == 1)
            printf("mr_jack is not hidden");
    }
}

void save(int round_num) {
    int round = round_num;
    FILE *ptr;
    ptr = fopen("test.bin", "wb");
    fwrite(&round, sizeof(round), 1, ptr);
    int map_char;
    for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 15; j++) {
            fwrite(&map[i][j].number, sizeof(int), 1, ptr);
        }
    }
}

void on_or_off_house() {
    int i = 0, j = 0;
    for (int h = 1; h <= 11; h++) {
        for (int k = 0; k < 15; k++) {
            if (map[h][k].block_number == 4) {
                j = h + 1;
                i = k + 1;
                map[j - 1][i - 1].on_off = 1;
                map[j][i - 1].on_off = 1;
                map[j - 2][i - 1].on_off = 1;
                map[j][i].on_off = 1;
                map[j][i - 2].on_off = 1;
                map[j - 1][i].on_off = 1;
                map[j - 1][i - 2].on_off = 1;
            }
        }
    }
}

int main() {
    int flag = 1;
    for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 15; j++) {
            map[i][j].number = flag;
            map[i][j].on_off = 0;
            flag++;
        }
    }
    printf("if you want play own map: enter 1\n");
    int own_map = 0;
    scanf("%d", &own_map);
    if (own_map == 1) {
        FILE *file = fopen("jack.txt", "r");
        int read;
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 15; j++) {
                fscanf(file, "%d ", &read);
                map[i][j].block_number = read;
            }
        }
    }

    start_game();
    while ((rouund) < 9) {
        on_or_off_house();
        print_map();
        printf("\n");
        struct node *list = random1();
        struct node *list1 = print_first_half_list(list);
        struct node *list2 = print_second_half_list(list);
        printf("round is %d\n", rouund);
        if (rouund % 2 == 1) {
            handle_odd_round(list1);
        } else {
            handle_even_round(list2);
        }
        state_mr_jack();
        save(rouund);
        rouund++;
    }
    return 0;
}